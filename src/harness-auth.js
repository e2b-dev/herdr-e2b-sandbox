...
export function renderAuthToml(plan) {
  ...
  for (const e of plan.entries) {
    ...
    if (e.kind === "session") t.session = { var: e.boxVar, expires: e.expires, value: e.value }
...
export function formatPlan(plan, file = AUTH_PATH) {
  ...
  if (plan.entries.length) {
    ...
    for (const e of plan.entries) {
      out.push(
        e.kind === "session"
          ? `  ${e.template.padEnd(10)} your signed-in session from ${e.from} — ${untilExpiry(e.expires)}, and it beats [templates.${e.template}.env]`
          : e.kind === "value"

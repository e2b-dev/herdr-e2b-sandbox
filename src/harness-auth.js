...
import { HARNESSES, remedyFor } from "./harnesses.js"
import { formatReport, probeAll } from "./harness-probe.js"

/** `~/x` → `/home/you/x`. The table writes paths the way their vendors document
 * them, which is with a tilde. */
const expand = (p) => (p.startsWith("~/") ? path.join(os.homedir(), p.slice(2)) : p)

const readTextFile = (p) => {
  try {
    return readFileSync(expand(p), "utf8")
  } catch {
    return null
  }
}
...
export function renderAuthToml(plan) {
  ...
  for (const e of plan.entries) {
    ...
    if (e.kind === "value") (t.env ||= {})[e.boxVar] = e.value

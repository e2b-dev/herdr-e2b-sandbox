...
| `fleet-name.js` | Fleet member naming, as pure functions: `sanitizeSlug` (fold any typed text into one legal ref component, `""` when nothing survives), `memberLabel`, `memberBranch` (`<prefix>/<slug>-<template>-<rand4>`; `$HERDR_E2B_FLEET_RAND` pins the suffix for tests). Also a CLI that `bin/e2b-fleet` calls for the whole plan — bash never builds a ref name, because with no fleet state the branch prefix *is* the fleet id. |

...
| `e2b-popup` | Opens that same dashboard entrypoint with Herdr's `popup` placement, leaving the underlying pane layout intact. Native `type = "popup"` bindings use `--render` to exec the dashboard directly, preserving plugin config/credential precedence without an action/CLI/API round trip. Both launch paths mark the disposable popup PTY with `E2B_DASH_POPUP=1`; the TUI consumes that marker and on a clean exit leaves its last screen/cursor intact until Herdr removes the popup. Ordinary panes and inline shell handoffs still restore their terminals. |
...
- **The picker annotates, never filters.** An auth mark changes what a template row
  *says* and never whether it is offered, where it sits, or what the picker returns
  ...
  configure later, or a base image that needs none. The mark is also a SOURCE and
...
`lib/chooser.sh` and `e2b-box auth`'s one confirmation are driven through a forked
**pty** — real keystrokes, real `/dev/tty` — which is why those blocks report a
*skip*, never a failure, when python3 is missing or is a macOS Command Line Tools
stub. No test ever creates a

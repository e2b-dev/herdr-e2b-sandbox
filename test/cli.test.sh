...
# name. This is ADR 0009's rule, and it is the one that has to hold on a real
...
# End to end against the real binary, with a fabricated HOME so nothing here reads
# the developer's own ~/.codex. Two claims a stub cannot make: the generated file
# holds a POINTER and not the key, and resolving that pointer at box-create time
# hands the box the key anyway. Skipped when codex is not installed — what this
# machine has is not a fixture.
if command -v codex >/dev/null 2>&1; then
  ...
  { grep -q '^path = ' "$fh/cfg/auth.toml" \
    ...
    && ok "a credential in a harness's own file is recorded as a pointer, never copied" \
  ...
  [ "$got" = "sk-fake-file-value" ] \
    ...
    || bad "pointer did not resolve (got '$got')"
else
  skip "file-sourced pointer (codex not installed here)"
fi
...
# ADR 0010: a borrowed session goes in verbatim, and it OUTRANKS the api-key branch.
